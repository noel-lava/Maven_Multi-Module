...
public class MapGrid implements Grid, Constants {
    private List<Map<MapKey, String>> gridMap;
    private Map<MapKey, String> lineMap;
    ...
    public MapGrid(String fileName) {
        this.fileName = fileName;
        readFile(fileName, GRID_FILE);
    }
    
    public MapGrid(int width, int height, String fileName) {
        this.fileName = (fileName == null)?GRID_DEFAULT:fileName;
        
        //Generate/Reset grid
        int size = width*height;
        this.gridMap = new ArrayList<Map<MapKey, String>>(height);
        String key;
        String val;
        String newLine = "";
        
        for(int ctr = 0; ctr < height; ctr++) {
            lineMap = new LinkedHashMap<MapKey, String>(width);
            for(int mapCtr = 0; mapCtr < width; mapCtr++) {
                key = getRandomString();
                val = getRandomString();
                
                lineMap.put(new MapKey(key), val);
                newLine += "(" + key + "," + val +") ";
            }
            //WRITE LINE ON FILE
            newLine += "\n";
	...
	public void edit(int option, int xIndex, int yIndex, String newVal) {
        ...
        // GET KEY
        MapKey key = getEntryByIndex(lineMap, yIndex).getKey();
        ...
        switch(option) {
            case 1:
                ...
                put(lineMap, yIndex, new MapKey(newVal), value);

                // REPRINT MAP, SAVE IN FILE
                // UPDATE FILE
                updateFile(writeMap(), false);
                System.out.println("Successfully edited key " + key.toString() + " to " + newVal);
                break;
            case 2:
    ...
    @Override
	public void addUpdate(int cols, MapKey[] keys, String[] vals) {
        String line = "";
        String err = null;
        Map<MapKey,String> newMap = new LinkedHashMap<MapKey,String>();
        
        for(int ctr = 0; ctr < cols; ctr++) {
            newMap.put(keys[ctr], vals[ctr]);
            line += "(" + keys[ctr].toString() + "," + vals[ctr] + ")";
        }
	...
	public void sort(int sort) {
        ...
        String line = "";
        int ctr = 0;
        for(Map<MapKey, String> map : gridMap) {
            Map<MapKey, String> newMap;
            if(sort == 2) {
                newMap = new TreeMap<MapKey, String>((Comparator<MapKey>)(MapKey k1, MapKey k2) -> k2.toString().compareTo(k1.toString()));
            } else {
                newMap = new TreeMap<MapKey, String>((Comparator<MapKey>)(MapKey k1, MapKey k2) -> k1.toString().compareTo(k2.toString()));
            }
            newMap.putAll(map);
            //map = new LinkedHashMap<MapKey, String>(newMap); 
            gridMap.set(ctr, newMap);
    ...
    private void put(Map<MapKey, String> input, int index, MapKey key, String value) {
        if (index >= 0 && index <= input.size()) {
            Map<MapKey, String> output=new LinkedHashMap<MapKey, String>();
            int i = 0;
            if (index == 0) {
                output.put(key, value);
                output.putAll(input);
            } else {
                for (Map.Entry<MapKey, String> entry : input.entrySet()) {
                    if (i == index) {
                        output.put(key, value);
                    }
    ...
    private void readFile(String file, String defaultFile){
        ...
        gridMap = new ArrayList<Map<MapKey, String>>();
		...
		try (Stream<String> lines = Files.lines(Paths.get(file))) {            
			lines.forEach(currentLine -> {
                ...
                lineMap = new LinkedHashMap<MapKey, String>();
                ...
                while(match.find()) {
                    ...
                    if(keyVal.length >= 2) {
                        lineMap.put(new MapKey(keyVal[0]), keyVal[1]);
                    }
                }
